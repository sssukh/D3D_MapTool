			 ...
			 const OutputPatch<HullOut, 4> quad)
{
	...
	// 사각형 패치 보간 (Bilinear)
	float4 posTop = lerp(quad[0].PosH, quad[1].PosH, uv.x);
	float4 posBottom = lerp(quad[3].PosH, quad[2].PosH, uv.x);
	float4 finalPos = lerp(posTop, posBottom, uv.y);

	float2 texTop = lerp(quad[0].TexC, quad[1].TexC, uv.x);
	float2 texBottom = lerp(quad[3].TexC, quad[2].TexC, uv.x);
	float2 finalTex = lerp(texTop, texBottom, uv.y);
