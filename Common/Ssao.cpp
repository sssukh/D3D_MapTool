...
void Ssao::BuildPso(D3D12_GRAPHICS_PIPELINE_STATE_DESC pBasePso)
{
	D3D12_GRAPHICS_PIPELINE_STATE_DESC drawNormalsPsoDesc = pBasePso;
    drawNormalsPsoDesc.VS =
    {
        reinterpret_cast<BYTE*>(NormalVS->GetBufferPointer()),
        NormalVS->GetBufferSize()
    };
    drawNormalsPsoDesc.PS =
    {
        reinterpret_cast<BYTE*>(NormalPS->GetBufferPointer()),
        NormalPS->GetBufferSize()
    };
    ...
    // PSO for SSAO.
    //
    D3D12_GRAPHICS_PIPELINE_STATE_DESC ssaoPsoDesc = pBasePso;
    ssaoPsoDesc.InputLayout = { nullptr, 0 };
    ssaoPsoDesc.pRootSignature = mSsaoRootSignature.Get();
    ssaoPsoDesc.VS =
    {
        reinterpret_cast<BYTE*>(SsaoVS->GetBufferPointer()),
        SsaoVS->GetBufferSize()
    };
    ssaoPsoDesc.PS =
    {
        reinterpret_cast<BYTE*>(SsaoPS->GetBufferPointer()),
        SsaoPS->GetBufferSize()
    };
    ...
    ssaoBlurPsoDesc.VS =
    {
        reinterpret_cast<BYTE*>(BlurVS->GetBufferPointer()),
        BlurVS->GetBufferSize()
    };
    ssaoBlurPsoDesc.PS =
    {
        reinterpret_cast<BYTE*>(BlurPS->GetBufferPointer()),
        BlurPS->GetBufferSize()
    };
    ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&ssaoBlurPsoDesc, IID_PPV_ARGS(&mBlurPso)));
}



void Ssao::BuildShaders()
{
    NormalVS = d3dUtil::CompileShader(L"C:\\MapTool\\Shaders\\DrawNormals.hlsl", nullptr, "VS", "vs_5_1");
    NormalPS= d3dUtil::CompileShader(L"C:\\MapTool\\Shaders\\DrawNormals.hlsl", nullptr, "PS", "ps_5_1");
    SsaoVS = d3dUtil::CompileShader(L"C:\\MapTool\\Shaders\\Ssao.hlsl", nullptr, "VS", "vs_5_1");
